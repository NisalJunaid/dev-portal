...
$statusOptionsForThisTicket = $ticket->isBug()
    ? [['value' => 'bug_pending', 'label' => 'Pending'], ['value' => 'bug_blocked', 'label' => 'Blocked'], ['value' => 'bug_completed', 'label' => 'Completed'], ['value' => 'rejected', 'label' => 'Rejected']]
    : [['value' => 'backlog', 'label' => 'Backlog'], ['value' => 'in_progress', 'label' => 'In Progress'], ['value' => 'task_blocked', 'label' => 'Blocked'], ['value' => 'task_completed', 'label' => 'Completed'], ['value' => 'rejected', 'label' => 'Rejected'], ['value' => 'next_sprint', 'label' => 'Move to Next Sprint']];
@endphp
<tr data-list-task-row data-ticket-id="{{ $ticket->id }}" data-ticket-type="{{ $ticket->type }}" data-current-section="{{ $ticket->listSectionKey($workType ?? 'tasks') }}" data-current-status="{{ $ticket->status }}" data-move-url="{{ route('tickets.inline-update', $ticket) }}" class="border-t">
...
<td class="px-4 py-2 relative overflow-visible"><button type="button" data-list-status-trigger class="badge badge-status cursor-pointer" @click="$root.openFieldMenu($event,{type:'status',ticketId:{{ $ticket->id }},endpoint:@js(route('tickets.inline-update', $ticket)),currentValue:@js($ticket->status),options:@js($statusOptionsForThisTicket)})"><span data-list-status-label>{{ $ticket->formattedStatus() }}</span></button></td>
<td class="px-4 py-2" data-list-assignee><button type="button" data-list-assignee-trigger class="inline-flex cursor-pointer items-center rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-700 hover:bg-slate-200" @click="$root.openFieldMenu($event,{type:'assignee',ticketId:{{ $ticket->id }},endpoint:@js(route('tickets.inline-update', $ticket)),currentValue:@js($ticket->assigned_to ? (string) $ticket->assigned_to : ''),options:@js($assigneeOptions)})"><span data-list-assignee-label>{{ $ticket->assignee?->name ?? 'Unassigned' }}</span></button></td>
</tr>
@endforeach
</tbody></table>
</div></div>
