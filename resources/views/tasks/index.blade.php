    ...
    >
        ...
        <section class="card flex items-center justify-between py-3">
            <div class="flex items-center gap-2 text-sm font-bold">
                <a href="{{ route('tasks.index', ['view' => 'list']) }}" class="rounded-xl px-3 py-1.5 {{ $activeView === 'list' ? 'bg-slate-900 text-white' : 'text-slate-600' }}">List</a>
                <a href="{{ route('tasks.index', ['view' => 'board']) }}" class="rounded-xl px-3 py-1.5 {{ $activeView === 'board' ? 'bg-slate-900 text-white' : 'text-slate-600' }}">Board</a>
                <a href="{{ route('tasks.index', ['view' => 'timeline']) }}" class="rounded-xl px-3 py-1.5 {{ $activeView === 'timeline' ? 'bg-slate-900 text-white' : 'text-slate-600' }}">Timeline</a>
            </div>
